...
The scaffold-model capability enables automatic generation of content model definitions by analyzing existing data files. This reduces manual configuration work when setting up new sites or onboarding existing Hugo/Quarto projects.
...
- `backend-architecture` - Service implementation patterns
- `dependency-injection` - Container and adapter patterns
- `adapters` - DialogAdapter interface for file selection
